...
class VideoExtractor:
    ...
    def extract_shopee_video(self, url: str) -> Optional[Dict]:
        try:
            ...
            # 1. Abre o link curto e segue até o final (resolve o br.shp.ee)
            res = session.get(url, headers=self.headers, allow_redirects=True, timeout=20)
            final_url = res.url
            
            # 2. SE FOR LINK DE VÍDEO (sv.shopee.com.br)
            if 'sv.shopee.com.br' in final_url:
                logger.info("Link de vídeo detectado. Buscando URL do arquivo...")
                # Procura o link que começa com cv.shopee.com.br dentro do código da página
            ...
            # 3. SE FOR LINK DE PRODUTO (Busca ShopID e ItemID)
            match = re.search(r'i\.(\d+)\.(\d+)', final_url)
            if not match:
                match = re.search(r'product/(\d+)/(\d+)', final_url)
            
            if not match:
                logger.error(f"Não encontrei IDs de produto em: {final_url}")
                return None
            ...
            logger.error(f"Erro crítico na Shopee: {e}")
            return None
